...
class Gridworld(Env):
    ...
    def render(self, mode='human'):
        ...
        self.viewer.draw_polyline( [(1, 1),
                                  ...
                                  (self.columns + 1, 1),
                                  (1, 1),
                                  ], color=(0, 0, 0) ) #color=(255.0, 255.0, 255.0) )

        # Draw grid lines
        if self.render_grid:
            for i in range(2,self.columns+1):
                self.viewer.draw_polyline( [(i, 1), (i, self.rows + 1)], color=(0, 0, 0) ) #color=(255.0, 255.0, 255.0) )
            for i in range(2,self.rows+1):
                self.viewer.draw_polyline( [(1, i), (self.columns + 1, i)], color=(0, 0, 0) ) #color=(255.0, 255.0, 255.0) )
