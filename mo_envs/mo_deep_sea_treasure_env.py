...
from mo_envs.mo_gridworld_base import MOGridworld
from mo_envs.gridworld_base import GridObject
...
if __name__=="__main__":
    my_grid = MODeepSeaTresureEnv(from_pixels=False)
    done = False
    my_grid.reset()
    available_actions = my_grid.action_space
    s, r, done, s_ = my_grid.step(my_grid.action_space.sample())
    next_state = s
    while not done:
        s, r, done, s_ = my_grid.step(my_grid.action_space.sample())  # What this returns...\
        print("State ", sum(s), len(s), sum(next_state-s), r, sum(r))
        next_state = s
